...
test('runs CLI request through model tool call and extension response', async (t) => {
  ...
  extension.on('message', (data) => {
    ...
    if (request.type === 'count-active-window-tabs')
      extension.send(
        JSON.stringify({ type: 'active-window-tabs-counted', requestId: request.requestId, windowId: 42, count: 12 }),
      );
  });
  ...
  cli.send(
    JSON.stringify({
      type: 'agent-request',
      requestId: 'cli-1',
      message: 'How many tabs are currently open in the active window?',
    }),
  );
  ...
  assert.deepEqual(response, {
    type: 'agent-response',
    requestId: 'cli-1',
    content: 'There are 12 tabs open in the active window.',
  });
